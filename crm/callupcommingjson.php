...
session_start(); 
	...
	if(isset($_SESSION['cstatus']))
	{
		...
		{
			$bb=" and res_complain_master.cstatus In ('Pending','In Process','Incomplete','Scheduled') ";
    ...
    END;"; */
			...
			$bb=" and res_complain_master.cstatus In ('Pending','Scheduled') and res_complain_master.cmid not in (select cmid from res_complainallocation_detail)";
		}else if($_SESSION['cstatus']=="Pending"){
			$bb=" and res_complain_master.cstatus In ('Pending','Scheduled') and res_complain_master.cmid in (select cmid from res_complainallocation_detail)";
		}
		else
		{
	...
	}else
	{
		...
		$dd = "";
		 ...
		 // echo $eve= "select * from res_complain_master,res_user_master where res_complain_master.customerid=res_user_master.umid and res_user_master.samid=$samid $aa $bb $cc $dd and res_user_master.utype='customer'   order by res_complain_master.cstatus desc,res_complain_master.cmupdatedon desc"; 
	$q = $db7->prepare("select * from res_complain_master,res_user_master where res_complain_master.customerid=res_user_master.umid and res_user_master.samid=$samid $aa $bb $cc $dd and res_user_master.utype='customer' and res_complain_master.servicetype='Service'  order by res_complain_master.cmid desc");
		...
		while($i<=$q->rowCount())
		{
			$rt = $q->fetch(PDO::FETCH_ASSOC);
					...
					while($rt22 = mysqli_fetch_assoc($re))
					{
						   ...
						   $servicetype=$rt22['servicetype'];
						    ...
						    $servicecontract='#'.$serviceno.'('.$servicetype.'-'.$servicestatus.')';
						   	$servicecontract="<a href='servicedetail.php?id=$servicemid' data-toggle='tooltip' data-placement='top' title='eye'   hidden-print' > $servicecontract <span class='fa fa-eye'  style='color:green;'></span></a>";
				...
				if($servicetype=="Service")
				{
					...
					$cmno="<span class='label label-sm label-success'>$rt[cmnowithprefix] <span class='fa fa-eye' style='color:gray'></span></span>";
				     $cmnonew="<a href='complaindetail.php?id=".$rt['cmid']."&page=Complain' title='Click Here' class=' hidden-print'> $cmno </a>";
				}
				else if($servicetype=="Complain")
				{
					$cmno="<span class='label label-sm label-danger'>$rt[cmnowithprefix] <span class='fa fa-eye' style='color:gray'></span> </span>";
					$cmnonew="<a href='complaindetail.php?id=".$rt['cmid']."&page=Complain' title='Click Here' class=' hidden-print'> $cmno</a>";
				}
				else if($servicetype=="Low")
				{
					$cmno="<span class='label label-sm label-success'>$rt[cmnowithprefix] <span class='fa fa-eye' style='color:gray'></span></span>";
					$cmnonew="<a href='complaindetail.php?id=".$rt['cmid']."&page=Complain' title='Click Here' class=' hidden-print'>$cmno </a>";
				}
