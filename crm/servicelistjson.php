...
session_start(); 
	...
	if(isset($_SESSION['cstatus']))
	{
		...
		{
		   $bb=" and res_complain_master.cstatus In ('Pending','In Process') ";
		}else if($_SESSION['cstatus']=="Unallocated"){
			
			$bb=" and res_complain_master.cstatus In ('Pending') and cmserviceengineerid = 0";
			 ...
			 */	
				...
				if($upriority=="High")
				{
					...
					$cmno="<span class='label label-sm label-danger'>$rt[cmnowithprefix]</span>";
				     $cmnonew="<a href='complaindetail.php?id=".$rt['cmid']."&page=Service' title='Click Here' > $cmno</a>";
				}
