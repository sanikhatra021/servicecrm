...
error_reporting(E_ERROR | E_PARSE);
	...
	if(mysqli_num_rows($re1) > 0)
	{
		while($rt1 = mysqli_fetch_assoc($re1))
		{
		 ...
		 $eve2 =  "SELECT * FROM  res_product_master where samid=$samid and ptype='Product'";
		...
		if(mysqli_num_rows($re2) > 0)
		{
			...
			while($rt2 = mysqli_fetch_assoc($re2))
			{
				...
				$pmname=$rt2['pmname'];
				$pmdesc=$rt2['pmdesc'];
				$pmremark=$rt2['pmremark'];
				
				$msg = "Success";
				$posts[] = array('p1'=> $result,'p2'=> $pmid,'p3'=> $pmname,'p4'=> $pmdesc,'p5'=> $pmremark);
			}
