...
error_reporting(E_ERROR | E_PARSE);
	...
	if(mysqli_num_rows($re1) > 0)
	{
		while($rt1 = mysqli_fetch_assoc($re1))
		{
		 ...
		 $eve2 =  "Select * from res_faq_master where samid=$samid";
		...
		if(mysqli_num_rows($re2) > 0)
		{
			...
			while($rt12 = mysqli_fetch_assoc($re2))
			{
				...
				$fmid=$rt12['fmid'];
				$fmproblem=$rt12['fmproblem'];
				$fmsolution=$rt12['fmsolution'];
				
				
				
				$msg = "Success";
				$posts[] = array('p1'=> $result,'p2'=> $fmid,'p3'=> $fmproblem,'p4'=> $fmsolution);
			}
