...
error_reporting(E_ERROR | E_PARSE);
	...
	if(mysqli_num_rows($re1) > 0)
	{
		...
		{
			  $eve2 =  "SELECT * FROM res_complain_master WHERE cstatus != 'Scheduled' AND cmid IN (SELECT cmid FROM res_complainallocation_detail WHERE serviceengid = $serviceengineerid) AND samid = $samid order by cmid IN (SELECT cmid FROM res_complain_detail where cactive=1) desc";
			  $re2 = mysqli_query($conn,$eve2);
		}
