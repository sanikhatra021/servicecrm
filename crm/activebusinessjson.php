...
session_start(); 
	...
	$q = $db7->prepare("select * from res_superadmin_master where sautype!='superadmin' and sacurrentplanstatus='Active'");
	//$q->bindValue(':p1', "$par1");
	//$q->bindValue(':p2',  "$par2");
	$q->execute();
	...
	if ($q->rowCount() > 0)
	{
		...
		while($i<=$q->rowCount())
		{
			...
			$posts[] = array('samid'=> $i++, 'safullname'=> $safullname, 'samobile'=> $samobile, 'saemailid'=> $saemailid,'saaddress'=> $saaddress,'saplanexpdate'=>$saplanexpdate,'view'=>$view,'saurl'=> $saurl,'saprefix'=> $saprefix,'op'=> $op);
		}
